    ...
    <el-card class="login-card">
      ...
      <el-form :model="form" label-width="90px">
        ...
        <template v-if="mode==='register'">
          ...
          <el-form-item label="车牌"><el-input v-model="form.vehiclePlate"/></el-form-item>
...
const form=reactive({username:'driver1',password:'123456',phone:'13900000000',realName:'李司机',vehiclePlate:'湘A12345',vehicleTypeId:null,serviceArea:'长沙'})
function switchRole(role){ if(role!=='DRIVER') window.location.href=roleUrls[role] }
async function submit(){loading.value=true;try{const data=mode.value==='login'?await http.post('/auth/login',{username:form.username,password:form.password,role:'DRIVER'}):await http.post('/auth/driver/register',form); auth.setAuth(data); router.push('/')}finally{loading.value=false}}
onMounted(async()=>{vehicles.value=await http.get('/common/vehicle-types'); form.vehicleTypeId=vehicles.value[0]?.id})
</script>
